...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [users] = await db.execute('SELECT name, email, credit_balance FROM public.users WHERE id = $1', [userId]);
    ...
    const [attemptStats] = await db.execute(
      `SELECT COUNT(*) as total_attempts,
       ...
       FROM public.attempts WHERE user_id = $1`, [userId]
    );

    const [latestRank] = await db.execute(
      `SELECT l.score, l.rank_position, tp.package_number
       FROM public.leaderboard l JOIN public.tryout_packages tp ON tp.id = l.tryout_package_id
  ...
  try {
    const [attempts] = await db.execute(
      `SELECT id, package_type, package_id, score, correct_count, wrong_count,
       ...
       FROM public.attempts WHERE user_id = $1 ORDER BY started_at DESC LIMIT 50`, [req.user.id]
    );
    res.json({ attempts });
  } catch (err) {
