...
router.post('/purchase/:id', auth, async (req, res) => {
  ...
  try {
    ...
    const debitId = `DEBIT-Q-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    await conn.execute(
      ...
      [req.user.id, debitId, 0, price, 'success', 'debit', `Pembelian Quiz: ${pkg.subtest} (Paket ${pkg.package_number})`]
    );
