...
router.get('/packages', auth, async (req, res) => {
  try {
    const [packages] = await db.execute('SELECT * FROM public.tryout_packages ORDER BY package_number');
    const [purchases] = await db.execute(
      'SELECT package_id FROM public.user_purchases WHERE user_id = $1 AND package_type = $2', [req.user.id, 'tryout']
    );
    let inProgress = [];
    try {
      const [res] = await db.execute(
        `SELECT a.package_id, a.id as attempt_id, tsp.subtest_name, tsp.time_remaining_seconds, tsp.started_at
         ...
         JOIN public.tryout_subtest_progress tsp ON tsp.attempt_id = a.id
    ...
    const [completed] = await db.execute(
      'SELECT package_id, id as attempt_id FROM public.attempts WHERE user_id = $1 AND package_type = $2 AND status = $3', [req.user.id, 'tryout', 'completed']
    );
...
router.get('/packages/:id/subtests', auth, async (req, res) => {
  try {
    const [subtests] = await db.execute(
      'SELECT * FROM public.tryout_subtests WHERE tryout_package_id = $1 ORDER BY sort_order', [req.params.id]
    );
    res.json({ subtests });
  } catch (err) {
...
router.post('/purchase/:id', auth, async (req, res) => {
  ...
  try {
    ...
    const [pkgs] = await conn.execute('SELECT * FROM public.tryout_packages WHERE id = $1', [packageId]);
...
router.post('/start/:packageId', auth, async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      ...
      const [qCount] = await db.execute('SELECT COUNT(*) as count FROM public.attempt_answers WHERE attempt_id = $1', [attemptId]);
      
      if (parseInt(qCount[0].count) > 0) {
        // Ensure subtest progress exists (for legacy attempts)
        const [progCheck] = await db.execute('SELECT id FROM public.tryout_subtest_progress WHERE attempt_id = $1', [attemptId]);
        if (progCheck.length === 0) {
          const [subs] = await db.execute('SELECT * FROM public.tryout_subtests WHERE tryout_package_id = $1 ORDER BY sort_order', [packageId]);
          for (const sub of subs) {
            await db.execute(
              'INSERT INTO public.tryout_subtest_progress (attempt_id, subtest_name, time_remaining_seconds) VALUES ($1, $2, $3)',
      ...
      await db.execute('DELETE FROM public.attempts WHERE id = $1', [attemptId]);
    }

    const [subtests] = await db.execute(
      'SELECT * FROM public.tryout_subtests WHERE tryout_package_id = $1 ORDER BY sort_order', [packageId]
    );
    ...
    const [result] = await db.execute(
      'INSERT INTO public.attempts (user_id, package_type, package_id, status) VALUES ($1, $2, $3, $4) RETURNING id',
      [req.user.id, 'tryout', packageId, 'in_progress']
    );
    ...
    const [pkg] = await db.execute('SELECT package_number FROM public.tryout_packages WHERE id = $1', [packageId]);
    const pkgNumber = pkg[0].package_number;

    for (const sub of subtests) {
      const [questions] = await db.execute(
        'SELECT id FROM public.questions WHERE subtest = $1 AND package_number = $2 AND question_type IN (\'tryout\', \'both\') ORDER BY RANDOM() LIMIT $3',
...
router.get('/attempt/:attemptId', auth, async (req, res) => {
  try {
    ...
    const [subtests] = await db.execute(
      'SELECT * FROM public.tryout_subtests WHERE tryout_package_id = $1 ORDER BY sort_order', [attempts[0].package_id]
    );

    let [progress] = await db.execute(
      'SELECT *, EXTRACT(EPOCH FROM (NOW() - started_at))::INT as elapsed FROM public.tryout_subtest_progress WHERE attempt_id = $1',
      [attemptId]
    );
...
router.post('/answer', auth, async (req, res) => {
  try {
    ...
    const [progress] = await db.execute(
      'SELECT *, EXTRACT(EPOCH FROM (NOW() - started_at))::INT as elapsed FROM public.tryout_subtest_progress WHERE attempt_id = $1 AND subtest_name = $2 AND status = $3',
...
router.post('/subtest/start', auth, async (req, res) => {
  try {
    ...
    await db.execute(
      "UPDATE public.tryout_subtest_progress SET status = 'in_progress', started_at = NOW() WHERE attempt_id = $1 AND subtest_name = $2",
      [attempt_id, subtest]
    );
    // Sync current_subtest in attempts
    await db.execute('UPDATE public.attempts SET current_subtest = $1 WHERE id = $2', [subtest, attempt_id]);
...
router.post('/submit/:attemptId', auth, async (req, res) => {
  ...
  try {
    ...
    const [existingEntry] = await conn.execute(
      'SELECT id, score FROM public.leaderboard WHERE tryout_package_id = $1 AND user_id = $2',
    ...
    if (existingEntry.length === 0 || score > parseFloat(existingEntry[0].score)) {
      if (existingEntry.length > 0) {
        await conn.execute(
          'UPDATE public.leaderboard SET score = $1, total_time_seconds = $2, attempt_id = $3 WHERE id = $4',
          [score, totalTime, attemptId, existingEntry[0].id]
        );
      } else {
        await conn.execute(
          'INSERT INTO public.leaderboard (tryout_package_id, user_id, attempt_id, score, total_time_seconds) VALUES ($1, $2, $3, $4, $5)',
      ...
      const [ranking] = await conn.execute(
        'SELECT id FROM public.leaderboard WHERE tryout_package_id = $1 ORDER BY score DESC, total_time_seconds ASC',
        [attempt.package_id]
      );
      for (let i = 0; i < ranking.length; i++) {
        await conn.execute('UPDATE public.leaderboard SET rank_position = $1 WHERE id = $2', [i + 1, ranking[i].id]);
      }
    }
...
router.get('/results/:attemptId', auth, async (req, res) => {
  try {
    ...
    const [rank] = await db.execute(
      'SELECT rank_position FROM public.leaderboard WHERE tryout_package_id = $1 AND user_id = $2',
